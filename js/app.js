...
const loadProducts = () => {
  ...
  showProducts(data); // pass API data this function
};


// show all product in UI 
const showProducts = (products) => {
  const allProducts = products.map((pd) => pd); // store all product by map method
  for (const product of allProducts) { // unpack all product by for loop
    const image = product.image; // product image url
    const div = document.createElement("div"); // creact product div
    div.classList.add("product"); // add product div class
    // start building product template
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p class="ratings">${product.rating.rate}<i class="fas fa-star"></i> (${product.rating.count})</p>
      ...
      `;
    // ending the building product template
    document.getElementById("all-products").appendChild(div); // push template in her father element
  }
};


// cart product length calculator
let count = 0; // item length store variable
const addToCart = (id, price) => {
  count = count + 1; // increment item length
  updatePrice("price", price); // pass price in updateprice function

  updateTaxAndCharge(); // call tax calculator function
  document.getElementById("total-Products").innerText = count; // set product length
};


// build a value conver machine by function
const getInputValue = (id) => {
  const element = document.getElementById(id).innerText; // catch value in field
  const converted = parseFloat(element); // conver into floating point number
  return converted; // back converted value
};


// main price update function
const updatePrice = (id, value) => {
  const convertedOldPrice = getInputValue(id); // catch old price
  const convertPrice = parseFloat(value); // recive current price and convert
  const total = convertedOldPrice + convertPrice; // calculate price
  document.getElementById(id).innerText = total.toFixed(2); // set price and presize tor decimal
};
...
const updateTaxAndCharge = () => {
  const priceConverted = getInputValue("price"); // push value in converter machine
  // start making some rules and set delivery charge
  ...
  // ending the making some rules and set delivery charge
};
...
const updateTotal = () => {
  ...
  // set total cost into two decimal format
  document.getElementById("total").innerText = grandTotal.toFixed(2);
};

loadProducts(); // this function load api data
